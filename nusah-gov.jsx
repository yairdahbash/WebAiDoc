...
const TONE_MAP = { formal: "רשמי", firm: "תקיף", polite: "מנומס", legal: "משפטי-למחצה" };
const TYPE_MAP = { appeal: "ערעור", request: "בקשה", complaint: "תלונה", official: "פנייה רשמית", custom: "מכתב חופשי" };
...
export default function NusahGov() {
  ...
  const [form, setForm] = useState({ docType: "", recipient: "", goal: "", story: "", facts: "", tone: "formal", fullName: "", email: "" });
  ...
  async function generate() {
    setLoading(true); setStep("result"); setOutput("");
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          model: "claude-sonnet-4-20250514", max_tokens: 1000,
          messages: [{ role: "user", content: `אתה עוזר מקצועי לכתיבת מכתבים רשמיים בעברית. כתוב מסמך ברור, מקצועי, ענייני ומשכנע.\n\nסוג: ${TYPE_MAP[form.docType]}\nמטרה: ${form.goal}\nנמען: ${form.recipient}\nתיאור: ${form.story}\nעובדות: ${form.facts || "לא צוינו"}\nטון: ${TONE_MAP[form.tone]}\n${form.fullName ? "שם: " + form.fullName : ""}\n${form.email ? "אימייל: " + form.email : ""}\n\nדרישות:\n- עברית תקינה ומקצועית\n- מבנה מכתב רשמי עם כותרת, גוף, סיום\n- אל תמציא חוקים או עובדות\n- פסקת סיום עם בקשה ברורה\n- "בכבוד רב," ומקום לחתימה` }]
        })
      });
      const d = await res.json();
      setOutput(d.content?.find(b => b.type === "text")?.text || "שגיאה.");
    } catch { setOutput("אירעה שגיאה. נסה שוב."); }
    setLoading(false);
  }

  async function refine(p) {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          model: "claude-sonnet-4-20250514", max_tokens: 1000,
          messages: [{ role: "user", content: `הנה מכתב:\n\n${output}\n\n${p}. החזר רק טקסט מעודכן.` }]
        })
      });
      const d = await res.json();
  ...
  const goTo = (s) => { setStep(s); setAnimIn(false); setTimeout(() => setAnimIn(true), 60); };
  ...
  return (
    <div dir="rtl" style={{ fontFamily: "'Heebo','Assistant',sans-serif", minHeight: "100vh", background: "#f4f6f9", color: "#1a2540" }}>
      ...
      <header style={{ background: "#1a2540", padding: "0 2rem", height: "64px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 100, boxShadow: "0 2px 16px rgba(0,0,0,.25)" }}>
        <div onClick={() => { setOutput(""); setPaid(false); goTo("home"); }} style={{ display: "flex", alignItems: "center", gap: "12px", cursor: "pointer" }}>
          {/* Logo mark */}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          {step !== "home" && (
            <button onClick={() => goTo(step === "result" ? "form" : "home")}
          ...
          <div style={{ background: "linear-gradient(160deg, #1a2540 0%, #1e3a6e 60%, #22488a 100%)", padding: "5rem 2rem 4rem", textAlign: "center", position: "relative", overflow: "hidden" }}>
            ...
            <div className={animIn ? "anim" : ""} style={{ position: "relative", zIndex: 1 }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: "6px", background: "rgba(42,122,226,.25)", border: "1px solid rgba(42,122,226,.4)", borderRadius: "20px", padding: "5px 16px", marginBottom: "1.75rem" }}>
                ...
                <span style={{ color: "#8cb8f0", fontSize: "12px", fontWeight: "500", letterSpacing: ".4px" }}>שירות פעיל · מייצר מסמכים ב-AI</span>
              </div>
              ...
              <p style={{ fontSize: "1.05rem", color: "#8cb8f0", maxWidth: "500px", margin: "0 auto 2.5rem", lineHeight: 1.8 }}>
                AI כותב עבורך ערעורים, בקשות ופניות רשמיות בנוסח מקצועי — תוך פחות מדקה
              </p>
              <button className="hover-lift" onClick={() => goTo("form")}
          ...
          <div style={{ maxWidth: "700px", margin: "0 auto", padding: "3rem 1rem 4rem" }}>
            ...
            <div style={{ marginBottom: "3rem" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(120px,1fr))", gap: "10px" }}>
                {DOC_TYPES.map(t => (
                  <div key={t.id} className="hover-lift" onClick={() => { upd("docType", t.id); goTo("form"); }}
            ...
            <div style={{ marginBottom: "3rem" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0" }}>
                {[
                  ...
                  { n: 2, title: "AI מייצר נוסח מקצועי", desc: "המסמך נכתב בשניות — רשמי, ענייני, משכנע", icon: "⚡" },
              ...
              <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: "14px", padding: "1.5rem", position: "relative", overflow: "hidden", minHeight: "110px" }}>
                ...
                <div style={{ display: "flex", gap: "5px", justifyContent: "center", marginTop: "1rem" }}>
                  {TESTIMONIALS.map((_, i) => (
                    <div key={i} style={{ width: i === tIdx ? "18px" : "6px", height: "6px", borderRadius: "3px", background: i === tIdx ? "#2a7ae2" : "#d0ddf0", transition: "all .3s", cursor: "pointer" }} onClick={() => setTIdx(i)} />
                  ))}
                </div>
              </div>
            </div>
      ...
      {step === "form" && (
        <div style={{ maxWidth: "700px", margin: "0 auto", padding: "2.5rem 1rem 4rem" }} className={animIn ? "anim" : ""}>
          ...
          <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: "16px", padding: "2rem", boxShadow: "0 2px 16px rgba(30,58,110,.06)" }}>
            ...
            <p style={{ fontSize: "13px", color: "#8a9ab5", marginBottom: "1.75rem" }}>כל שדה מסייע ל-AI לכתוב נוסח מדויק ומשכנע יותר</p>
            ...
            <div style={{ marginBottom: "1.5rem" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: "6px" }}>
                {DOC_TYPES.map(t => {
                  ...
                  return (
                    <div key={t.id} onClick={() => upd("docType", t.id)}
            ...
            {[
              ...
              <div key={f.k} style={{ marginBottom: "1.25rem" }}>
                ...
                {f.multi ? (
                  <textarea rows={3} value={form[f.k]} placeholder={f.ph} onChange={e => upd(f.k, e.target.value)}
                  ...
                  <input type="text" value={form[f.k]} placeholder={f.ph} onChange={e => upd(f.k, e.target.value)}
            ...
            <div style={{ marginBottom: "1.5rem" }}>
              ...
              <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                {TONES.map(t => (
                  <button key={t.id} className="pill-hover" onClick={() => upd("tone", t.id)}
            ...
            <button onClick={canGen ? generate : undefined}
              ...
              {canGen ? "✦  צור מסמך מקצועי" : "מלא את השדות הנדרשים"}
            </button>
          </div>
        </div>
      )}
      ...
      {step === "result" && (
        <div style={{ maxWidth: "700px", margin: "0 auto", padding: "2.5rem 1rem 4rem" }} className={animIn ? "anim" : ""}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.25rem" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "6px", background: loading ? "#fff8e1" : paid ? "#f0fdf4" : "#eff6ff", border: `1px solid ${loading ? "#fde68a" : paid ? "#bbf7d0" : "#bfdbfe"}`, borderRadius: "20px", padding: "5px 12px" }}>
              {loading
                ? <><div style={{ width: "8px", height: "8px", borderRadius: "50%", border: "2px solid #f59e0b", borderTopColor: "transparent", animation: "spin .7s linear infinite" }} /><span style={{ fontSize: "12px", color: "#92400e" }}>מייצר...</span></>
                : paid
                ? <><div style={{ width: "7px", height: "7px", borderRadius: "50%", background: "#22c55e" }} /><span style={{ fontSize: "12px", color: "#15803d" }}>מוכן לשימוש</span></>
                : <><div style={{ width: "7px", height: "7px", borderRadius: "50%", background: "#3b82f6", animation: "pulse 2s infinite" }} /><span style={{ fontSize: "12px", color: "#1d4ed8" }}>תצוגה מקדימה</span></>
              }
            </div>
          </div>
          ...
          <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: "16px", boxShadow: "0 2px 20px rgba(30,58,110,.07)", overflow: "hidden" }}>
            ...
            <div style={{ background: "linear-gradient(90deg,#1e3a6e,#2a7ae2)", padding: "10px 1.5rem", display: "flex", alignItems: "center", gap: "8px" }}>
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><rect x="1" y="1" width="12" height="12" rx="2" stroke="rgba(255,255,255,.6)" strokeWidth="1.2" fill="none"/><line x1="3" y1="4.5" x2="11" y2="4.5" stroke="rgba(255,255,255,.6)" strokeWidth="1" strokeLinecap="round"/><line x1="3" y1="7" x2="9" y2="7" stroke="rgba(255,255,255,.6)" strokeWidth="1" strokeLinecap="round"/><line x1="3" y1="9.5" x2="7" y2="9.5" stroke="rgba(255,255,255,.6)" strokeWidth="1" strokeLinecap="round"/></svg>
              <span style={{ fontSize: "12px", color: "rgba(255,255,255,.75)", fontWeight: "500" }}>
                {TYPE_MAP[form.docType] || "מסמך"} · {TONE_MAP[form.tone]}
              </span>
            </div>

            <div style={{ padding: "1.75rem", minHeight: "220px", position: "relative" }}>
              {loading ? (
                ...
                <div style={{ fontSize: "14.5px", lineHeight: "1.95", whiteSpace: "pre-wrap", color: "#2d3a52", fontFamily: "'Heebo',sans-serif" }}>{output}</div>
              ) : (
                <>
                  <div style={{ fontSize: "14.5px", lineHeight: "1.95", color: "#2d3a52", fontFamily: "'Heebo',sans-serif", WebkitMaskImage: "linear-gradient(to bottom, black 30%, transparent 100%)", maskImage: "linear-gradient(to bottom, black 30%, transparent 100%)", whiteSpace: "pre-wrap" }}>
                    {output.slice(0, 480)}
                  </div>
                  {/* Paywall */}
                  <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "4rem 2rem 2rem", background: "linear-gradient(to top,#fff 60%,transparent)", textAlign: "center" }}>
                    <div style={{ background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: "12px", padding: "1.5rem", maxWidth: "340px", margin: "0 auto" }}>
                      <div style={{ fontSize: "22px", marginBottom: "6px" }}>🔒</div>
                      <p style={{ fontSize: "14px", fontWeight: "600", color: "#1e3a6e", marginBottom: "4px" }}>המסמך המלא מוכן</p>
                      <p style={{ fontSize: "12px", color: "#6b8ab5", marginBottom: "1rem" }}>פתח אותו ב-19 ₪ · תשלום מאובטח</p>
                      <button onClick={() => setPaid(true)}
                        style={{ background: "linear-gradient(135deg,#1e3a6e,#2a7ae2)", color: "#fff", border: "none", borderRadius: "10px", padding: "11px 28px", fontSize: "15px", fontWeight: "700", cursor: "pointer", width: "100%", boxShadow: "0 4px 16px rgba(30,58,110,.3)" }}>
                        פתח מסמך מלא — 19 ₪
                      </button>
                    </div>
                  </div>
          ...
          {!loading && paid && (
            <div style={{ marginTop: "1rem" }}>
              <div style={{ display: "flex", gap: "8px" }}>
                <button className="copy-hover" onClick={() => { navigator.clipboard.writeText(output); setCopied(true); setTimeout(() => setCopied(false), 2000); }}
                  style={{ flex: 1, padding: "11px", fontSize: "13px", fontWeight: "600", background: copied ? "#2a7ae2" : "#fff", color: copied ? "#fff" : "#1e3a6e", border: "1.5px solid #2a7ae2", borderRadius: "10px", cursor: "pointer", transition: "all .15s" }}>
                  {copied ? "✓ הועתק!" : "העתק טקסט"}
                </button>
                <button onClick={() => { setOutput(""); setPaid(false); goTo("form"); }}
                  style={{ flex: 1, padding: "11px", fontSize: "13px", fontWeight: "600", background: "#fff", color: "#6b7a99", border: "1.5px solid #e2e8f0", borderRadius: "10px", cursor: "pointer" }}>
                  מסמך חדש
                </button>
              </div>
              <div style={{ marginTop: "1rem" }}>
                ...
                <div style={{ display: "flex", gap: "7px", flexWrap: "wrap" }}>
                  {[
                    ...
                    <button key={r.l} className="refine-hover" onClick={() => refine(r.p)} disabled={loading}
          ...
          {!loading && !paid && (
            <button onClick={() => goTo("form")}
              style={{ marginTop: "1rem", padding: "10px 20px", fontSize: "13px", background: "#fff", color: "#6b7a99", border: "1.5px solid #e2e8f0", borderRadius: "10px", cursor: "pointer" }}>
              ← ערוך פרטים
            </button>
          )}
        </div>
